...
public interface UsersProfileRepository extends JpaRepository<UsersProfile, Long> {
	...
	@Query("select p.profileUrl from UsersProfile up join up.profile p where up.user.nickname = :nickname and p.profileType = 'CUSTOMIZATION' and p.thumbnail = false")
